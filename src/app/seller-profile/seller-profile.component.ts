...
export class SellerProfileComponent implements  OnInit, AfterViewInit {
...
,'likeUnlike').subscribe(res => {
  if(res.status){
    this.feeds[i].isIamLiking=!data.isIamLiking;
	this.feedlist();
  ...
  clearall() {
	...
	feedlist(){
      ...
      this.CrudService.list(params1, 'sellerfeeds').subscribe(data => {
  ...
  apicall() {
	...
	this.service_url=this.route.snapshot.params.url;
    var params = {
      "page": 1,
      "sort_by": 1,
      "type": this.type,
    ...
    this.CrudService.list(params, 'sellerProfile').subscribe(data => {
